...
class HailoAsyncInference:
    ...
    def callback(self, completion_info, all_bindings, out_queue):
        ...
        outputs = {}
        for name in self.output_names:
            outputs[name] = [binding.output(name).get_buffer() for binding in all_bindings]

        # ★ 꽉 차면 하나 버리고 새걸 넣는다 (드롭은 가장 오래된 것)
        try:
            out_queue.put_nowait(outputs)
        except queue.Full:
            try:
                _ = out_queue.get_nowait()     # drop oldest
            except Exception:
                pass
            try:
                out_queue.put_nowait(outputs)
