...
def beep_once():
    """Tru multiple ways to beep once; return True if started."""
    candidates = [
    ...
    for cmd in candidates:
        try:
            subprocees.Popen(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            return True
...
def main():
    ...
    ap.add_argument("--require-sig", action="store_true", help="drop packets without valid HMAC signature")
    args = ap.parse_args()
    ...
    while True:
        ...
        try:
            ...
            # optional beep on critical events
